...
export const TestListItem = ({ node, depth, id, runTest }: TestListItemProps) => {
  ...
  const isPreviouslyRunTest =
    typeof window !== 'undefined' && sessionStorage.getItem('twd-last-run-test-name') === node.name;
  ...
  useEffect(() => {
    ...
    const testJustFinished =
      previousStatusRef.current === 'running' && (node.status === 'pass' || node.status === 'fail');
    ...
    >
      ...
      >
        <span className="twd-test-item-name">
          <SkipOnlyName id={id} name={node.name} skip={node.skip} only={node.only} />
        </span>
        <button
          onClick={() => runTest(id)}
