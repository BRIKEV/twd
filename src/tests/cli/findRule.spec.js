...
describe('findRule', () => {
  ...
  it('matches by path (rule url starts with /)', () => {
    const rules = [{ method: 'GET', url: '/auth/v1/token?grant_type=password', alias: 'a' }];
    expect(
      findRule('GET', 'http://127.0.0.1:54321/auth/v1/token?grant_type=password', rules),
    ).toEqual(rules[0]);
    expect(
      findRule('GET', 'http://127.0.0.1:54321/auth/v1/token?grant_type=refresh', rules),
    ).toBeUndefined();
    expect(findRule('GET', 'http://127.0.0.1:54321/other', rules)).toBeUndefined();
  });

  it('matches by regex string (starts with ^)', () => {
    const rules = [{ method: 'GET', url: '^https://foo\\.com/.*$', alias: 'a', urlRegex: true }];
  ...
  it('matches by regex string (wrapped in /.../)', () => {
    const rules = [{ method: 'GET', url: /users.*/, alias: 'a', urlRegex: true }];
  ...
  it('is case-insensitive for method', () => {
    const rules = [{ method: 'get', url: 'https://foo.com', alias: 'a' }];
    expect(findRule('GET', 'https://foo.com', rules)).toEqual(rules[0]);
    expect(findRule('get', 'https://foo.com', rules)).toEqual(rules[0]);
    expect(findRule('Get', 'https://foo.com', rules)).toEqual(rules[0]);
  });
  ...
  it('returns undefined if no match', () => {
    const rules = [{ method: 'GET', url: '/api', alias: 'a' }];
  ...
  it('should match if the url is a file with extension and the rule url is a file with extension', () => {
    const rules = [{ method: 'GET', url: '/api.json', alias: 'a' }];
  ...
  it('should match API paths with version numbers containing dots', () => {
    const rules = [{ method: 'GET', url: '/api.v2/users', alias: 'a' }];
  ...
  it('should match versioned API endpoints (e.g., /api.v2)', () => {
    const rules = [{ method: 'GET', url: '/api.v2', alias: 'a' }];
  ...
  it('should match semantic version paths (e.g., /v1.2.3/endpoint)', () => {
    const rules = [{ method: 'GET', url: '/v1.2.3/endpoint', alias: 'a' }];
  ...
  it('should still correctly identify actual files vs versioned APIs', () => {
    const rules = [{ method: 'GET', url: '/api/data', alias: 'api' }];
  ...
  describe('boundary-aware matching', () => {
    it('does not match overlapping path segments (wallet vs wallet-transactions)', () => {
      ...
      expect(
        findRule(
          'GET',
          'http://localhost/v1/travelers/123/wallet-transactions?page=1&page_size=10',
          rules,
        ),
      ).toEqual(rules[1]);
    ...
    it('matches when URL ends with rule URL (end of string boundary)', () => {
      const rules = [{ method: 'GET', url: '/api/users', alias: 'users' }];
      expect(findRule('GET', 'http://localhost/api/users', rules)).toEqual(rules[0]);
    });

    it('matches when followed by ? (query param boundary)', () => {
      const rules = [{ method: 'GET', url: '/api/users', alias: 'users' }];
      expect(findRule('GET', 'http://localhost/api/users?page=1&limit=10', rules)).toEqual(
        rules[0],
      );
    });
    ...
    it('matches rule with query params followed by & (ampersand boundary)', () => {
      const rules = [{ method: 'GET', url: '/api/users?page=1', alias: 'users' }];
      expect(findRule('GET', 'http://localhost/api/users?page=1&limit=10', rules)).toEqual(
        rules[0],
      );
    });
    ...
    it('does not match sub-resource paths (travelers/123 vs travelers/123/billing-details)', () => {
      ...
      expect(findRule('GET', 'http://localhost/v1/travelers/123/billing-details', rules)).toEqual(
        rules[1],
      );
    ...
    it('does not match partial path segments', () => {
      const rules = [{ method: 'GET', url: '/api/item', alias: 'item' }];
    ...
    it('does not match when rule URL is a substring mid-segment', () => {
      const rules = [{ method: 'GET', url: '/user', alias: 'user' }];
    ...
    it('allows substring matching within query strings', () => {
      ...
      expect(findRule('GET', 'https://api.tvmaze.com/search/shows?q=friends', rules)).toEqual(
        rules[0],
      );
    ...
    it('allows substring matching for partial query param values', () => {
      const rules = [{ method: 'GET', url: '/api/data?filter=active', alias: 'data' }];
      expect(findRule('GET', 'http://localhost/api/data?filter=active&sort=name', rules)).toEqual(
        rules[0],
      );
    });

    it('still applies boundary check in path even when URL has query string', () => {
      const rules = [{ method: 'GET', url: '/api/user', alias: 'user' }];
