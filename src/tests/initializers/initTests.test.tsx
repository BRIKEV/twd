...
describe('initTests', () => {
  ...
  it('should run all test modules and then initialize sidebar', async () => {
    ...
    expect(initSidebar).toHaveBeenCalledWith({
      Component: <TWDSidebar open={true} position="right" />,
      createRoot,
    });
  });
  ...
  it('should handle empty testModules gracefully', async () => {
    ...
    await expect(
      initTests(fakeModules, <TWDSidebar open={false} position="right" />, createRoot),
    ).resolves.not.toThrow();
    expect(initSidebar).toHaveBeenCalledWith({
      Component: <TWDSidebar open={false} position="right" />,
      createRoot,
    });
  });
});
